...
dropping metadata. Transcript history is requested only when transcript-backed
search is first used. Protocol v2 and queryless tabs fail closed and must use
the explicit reload flow; mixed v2/v3 operation is unsupported.

Each live PTY owns an in-memory `@xterm/headless` twin behind
`server-capture.js`. It keeps 5,000 scrollback lines, serializes at most 1,000
lines and 1 MiB, and supplies transcript/menu/preview/ask/Autopilot capture
without browser buffer uploads. Raw PTY output remains memory-only and is
sequenced immediately; only WebSocket delivery is batched (16 ms or 32 KiB).
Slow clients stop at a 1 MiB send backlog and recover by cursor/snapshot after
draining below 256 KiB. One healthy resize-owning renderer answers terminal
queries; otherwise the headless twin answers, so a query reaches the PTY once.
...
20-500 columns by 5-300 rows are accepted. Touch clients keep one renderer and
dispose it when switching or backgrounding; desktop creates renderers lazily
and retains them. Only the visible renderer is fit, and PTY resize follows 120
ms of stable dimensions.

The release client is generated by `npm run build:client`. It bundles the app,
xterm core, Fit addon, and CSS as ES2020 ESM under `public/build/`; WebGL is a
lazy content-hashed chunk. Generated assets and their Brotli/gzip sidecars are
...
| Metric | Before | After |
| --- | ---: | ---: |
| Idle reconnect control bytes | 21,657 | 25,264 |
| Automatic terminal replay bytes | 40,293 | 0 |
| WebSocket frames during measured reconnect | 18 | 9 |
| Maximum server send backlog | not exposed | 20,871 bytes |
| Touch renderers / WebGL contexts | 10 / 10 | 1 / 1 |
| Terminal fits / PTY resizes | 11 / 11 | 2 / 1 |
| Cold critical requests | 35 local modules | 6 |
| Cold critical compressed bytes | 227,403 Brotli | 164,318 encoded |
| Cold terminal usable | not profiled | 833 ms |
| WebSocket-open to terminal subscribed | not profiled | 109 ms |
| Long tasks / duration (representative run) | 4 / 1,696 ms | 4 / 453 ms |
...
offline recovery, lifecycle storms, cursor gaps, OSC 52 replay suppression,
one renderer on touch, and retained lazy renderers on desktop. These isolated
measurements do not replace the real Android acceptance pass described below.
